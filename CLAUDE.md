...
- **L'enum `user_role` n'a PAS de valeur `'utilisateur'`.** Le rôle EST le métier (`enseignant`, `secretaire`, `cpe`, `comptable`, `surveillant`, `directeur`, `proviseur`, `parent`, `eleve`, `infirmier`, `responsable_cantine`, + `super_admin`, `admin_groupe`). La constante `AppConstants.roleUtilisateur` a été **RETIRÉE** (revérifié le 2026-08-31) et `test/mention_source_unique_test.dart` échoue si elle revient. Pour un test de rôle : `_isStaffRole`. Un bug historique (`role == 'utilisateur'`) avait tué la synchro du personnel — résolu le 2026-06-06.
