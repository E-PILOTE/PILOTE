...
« Personnel scolaire » = tout rôle SAUF `super_admin` et `admin_groupe`. La fonction de référence est `isStaffRole(role)` dans `lib/services/powersync/powersync_service.dart` :
```dart
...
- **L'enum `user_role` n'a PAS de valeur `'utilisateur'`.** Le rôle EST le métier (`enseignant`, `secretaire`, `cpe`, `comptable`, `surveillant`, `directeur`, `proviseur`, `parent`, `eleve`, `infirmier`, `responsable_cantine`, + `super_admin`, `admin_groupe`). La constante `AppConstants.roleUtilisateur` a été **RETIRÉE** (revérifié le 2026-08-31) et `test/mention_source_unique_test.dart` échoue si elle revient. Pour un test de rôle : `isStaffRole`. Un bug historique (`role == 'utilisateur'`) avait tué la synchro du personnel — résolu le 2026-06-06.
...
- ⚠️ **Ne JAMAIS ajouter `42703` (ni `42P01`, `42804`) à `_fatalResponseCodes`.** Un code fatal fait *compléter* la transaction : les écritures de l'école seraient **jetées**. Un désaccord de schéma doit rester rejoué (rien n'est perdu) et se SIGNALER — `sync_failures.kind = 'blocage'`, cf. `docs/memoire/blocage-de-file-visible.md`.
- ⚠️ **DEUX instances PowerSync.** Le parc utilise **`6a185943234fa2bf51a66759` (Production)** ; `6a185941234fa2bf51a66757` est **Development** — c'est pourtant celle que citent toutes les vieilles notes. Déployer sur la mauvaise réussit et ne touche personne. Voir `docs/memoire/powersync-deux-instances.md`.
...
- **Mentions — source unique et UNIQUE exemplaire : `lib/core/utils/mention.dart`** (`mentionFor`). Excellent ≥18, Très Bien ≥16, Bien ≥14, Assez Bien ≥12, Passable ≥10, Insuffisant <10 ; barre de réussite 10/20. ⚠️ `get_mention()` **n'existe plus en base** — supprimée avec les constantes mortes `AppConstants.seuil*` le 2026-08-25 (migration 0117), parce qu'aucune des deux n'avait d'appelant, donc aucun test, donc une dérive silencieuse : le barème avait glissé de 2 points côté bulletins et 8/20, une note d'échec, ressortait « Passable ». Ne jamais recréer de copie « pour le serveur » : l'app est offline-first, elle doit savoir calculer une mention sans réseau, l'autorité est donc dans le Dart. Devise XAF (FCFA).
- **Vocabulaire d'évaluation — deux colonnes, deux sens, ne pas les confondre** : `bulletins.decision` porte la **distinction du conseil de classe**, par code (`felicitations`, `encouragements`, `tableau_honneur`, `avertissement_travail`, `avertissement_conduite`, `blame` — cf. `conseils_provider.dart`). Le **verdict annuel** de passage vit ailleurs, dans `class_enrollments.promotion_decision` (`passe` / `redouble` / `reoriente`). Écrire « Admis en classe supérieure » dans `bulletins.decision` ne lève aucune erreur : `awardFor()` renvoie simplement `null`, le bulletin s'affiche sans distinction et les compteurs du module Conseils restent à zéro.
