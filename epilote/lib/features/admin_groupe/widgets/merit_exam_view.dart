...
import 'merit_table.dart';
import 'student_dossier_dialog.dart';

// ════════════════════════════════════════════════════════════════════════════
//  LAURÉATS AUX EXAMENS D'ÉTAT — résultats ENREGISTRÉS, jamais calculés ici.
//
//  La plateforme ne fait pas passer les examens : elle transmet à la DEC la
//  liste des candidats des classes d'examen, et la DEC proclame les résultats.
//  Ce tableau les restitue quand ils ont été enregistrés — il ne les produit
//  pas. C'est la différence de fond avec les classes de passage, où la
//  plateforme calcule elle-même les moyennes.
//
//  Quand ils existent, ces résultats sont la SEULE base comparable entre
//  établissements : même sujet, même jury, même barème.
// ════════════════════════════════════════════════════════════════════════════
class MeritExamView extends ConsumerStatefulWidget {
  const MeritExamView({super.key});

  @override
  ConsumerState<MeritExamView> createState() => _ExamState();
}

class _ExamState extends ConsumerState<MeritExamView> {
  @override
                  ...
                  rows: [
                    for (final r in rows.where((r) => r.rank <= 3))
                      PodiumItem(
                        ...
                        subtitle: r.entry.filiere ?? 'Enseignement général',
                        caption: r.exAequo ? 'ex æquo' : r.entry.mention,
                        exAequo: r.exAequo,
                      ),
                  ],
                  onTap: (p) => _openLaureate(
                      rows.firstWhere((r) =>
...
// ─── Ce que cet onglet ne couvre pas ────────────────────────────────────────
    ...
    return AdminCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const AdminSectionTitle(
          ...
          subtitle: 'Elles relèvent de l\'autre onglet',
        ),
        const SizedBox(height: 12),
        Text(
          'Ce classement ne concerne que les classes d\'examen — celles dont la '
          'plateforme transmet la liste des candidats à la DEC. Les autres '
          'classes passent au niveau supérieur sur le travail de l\'année : '
          'leurs meilleurs élèves sont classés dans l\'onglet « Classes de '
          'passage », trimestre par trimestre, à partir des moyennes que la '
          'plateforme calcule elle-même.',
