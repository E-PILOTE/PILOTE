...
- [🔇 Un poste bloqué le DIT maintenant](blocage-de-file-visible.md) — `42703` rejoué à l'infini en silence = plus rien ne remonte ; `sync_failures.kind = 'blocage'`, effacement automatique ; ⚠️ NE JAMAIS rendre `42703` fatal (ça jetterait les écritures) ; ✅ une colonne ajoutée à une table `localOnly` ne perd RIEN (PowerSync stocke `(id, data)` + une vue) — vérifié le 01/09 ; ⚠️ la sonde 0166 annonçait 86 tables, en listait 67, 87 descendent → REMPLACÉE par 0169 (UPDATE **et** DELETE, liste tenue par un test) ; aucun UPDATE ne lève JAMAIS
- [🚨 CI bloquée par la FACTURATION GitHub + publication manuelle](chaine-livraison-windows.md) — depuis le 30/08 aucune exécution ne démarre (3-5 s, « payments have failed ») ; v3.4.1 puis **v3.4.2** construites et publiées à la main ; ⚠️ ISCC est en profil utilisateur et se lance en **PowerShell** (Git Bash convertit `/DAppVersion=`) ; ⚠️ PS 5.1 double-encode les notes ; ⚠️ **jamais `dist/*.exe`** — le dossier porte 6 installateurs
...
- [🔏 Signature de code — ÉCARTÉE](../CERTIFICAT_SIGNATURE.md) — décision du 01/09 : on ne signe pas ; SmartScreen ne frappe que sur Mark-of-the-Web, donc **navigateur seulement** (clé USB et mise à jour intégrée n'en portent pas — VÉRIFIÉ) ; la garantie est l'**empreinte SHA-256**, déjà vérifiée par l'app ; ⚠️ reste « Éditeur inconnu » en UAC + risque de faux positifs antivirus ; implémentation HSM conservée au commit `9db2838`
...
- [📡 Relevé du parc — quelle version tourne où](releve-du-parc.md) — mig 0150 ; ⚠️ **le chiffre qui décide est `jamais_signale`, pas `a_jour`** (les builds < 24 ne savent pas se signaler) ; seuil `0146` = build **24**, ni 21 ni 23 ; table HORS PowerSync ; **01/09 : 1 seule installation déclarée** (la machine de dev) et les **10 sessions jamais ouvertes datent TOUTES d'avant le 29/08** → aucune n'a pu se signaler ; ⚠️ `profiles.last_login` est nulle sur 344 comptes (colonne morte, le chiffre vient de `auth.users`)
