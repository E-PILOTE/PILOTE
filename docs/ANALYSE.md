...
3. **Validation notes** : Directeur valide avant publication → notification
   > 🟢 **Tenu depuis le 2026-08-29 — sans Firebase, et c'est mieux ainsi.**
   >
   > La validation est appliquée depuis le 2026-08-27 : le bouton « Publier »
   > exige le droit `validate` (que l'enseignant n'a pas), et la base le refuse
   > aussi — RLS `bulletins`, `WITH CHECK` sur `status = 'published'`
   > (migrations `0118`/`0119`). Avant cela, un enseignant publiait lui-même.
   >
   > **La notification, elle, ne passera pas par FCM.** Décision du 2026-08-29 :
   > la plateforme n'a qu'un fournisseur, Supabase. Ce n'est pas un ajournement,
   > c'est un remplacement — et sur ce terrain précis, par mieux :
   >
   > | | FCM | table `notifications` + PowerSync |
   > |---|---|---|
   > | Windows (les écoles) | **jamais** | oui |
   > | Poste éteint pendant l'événement | perdu | **retrouvé au démarrage** |
   > | Hors ligne | rien | oui, c'est la base locale |
   > | Fournisseur supplémentaire | Firebase | aucun |
   >
   > `firebase_messaging` ne supporte qu'Android et iOS. Les écoles travaillent
   > sur des postes Windows partagés — secrétariat, surveillance et direction
   > s'y succèdent : le push n'aurait **jamais sonné là où le travail se fait**.
   > La cloche, elle, est stockée et non diffusée : elle survit à l'extinction
   > du poste, ce qu'aucun push ne fait.
   >
   > Ce qui existe et fonctionne : la table `notifications` (121 lignes), les
   > déclencheurs `notify_on_announcement` et `notify_on_message`, la
   > synchronisation PowerSync, la cloche et son tiroir dans l'en-tête. Le
   > personnel scolaire la lit **en local** (`_notificationsOffline`) : elle ne
   > se vide pas quand le réseau tombe.
   >
   > ✅ **Fait le 2026-08-29** : les deux colonnes Firebase mortes ont quitté le
   > client. `profiles.fcm_token` est une CLÉ D'APPAREIL — qui la détient peut
   > faire sonner le téléphone d'un collègue — et elle n'aura jamais de
   > producteur ; la laisser dans le schéma local, c'était garder une colonne
   > qui attend un `SELECT *` pour se répandre. Elle et
   > `notifications.fcm_message_id` sont retirées du schéma PowerSync et de
   > `ProfileModel`, ce qui est **le préalable au DROP** : un poste qui écrirait
   > encore vers une colonne disparue produirait un `42703`, que
   > `_fatalResponseCodes` ne reconnaît pas — le connecteur rejouerait le lot
   > sans fin et la synchro de ce poste serait bloquée en silence. La migration
   > `0146` attend donc que **tout** le parc soit passé sur le build ≥ 3.3.1.
   > Garde : `test/notification_sans_firebase_test.dart`.
   >
   > 📅 **Les familles restent hors périmètre pour l'instant.** Sur 344 profils,
   > zéro parent et zéro élève ; `/user/espace-parent` est un écran « bientôt
   > disponible » et la plateforme compte 2 tuteurs. L'espace élève et parent
   > est **planifié en dernier** (décision du 2026-08-29) : le prévenir n'a de
   > sens qu'une fois qu'il a un compte.
